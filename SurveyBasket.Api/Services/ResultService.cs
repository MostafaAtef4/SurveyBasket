...
public class ResultService(ApplicationDbContext context) : IResultService
{
    ...
    public async Task<Result<PollVotesResponse>> GetPollVotesAsync(int pollId, CancellationToken cancellationToken = default)
    {
        ...
        var pollVotes = await _context.Polls.Where(x => x.Id == pollId)
    ...
    public async Task<Result<IEnumerable<VotesPerDayRespone>>> GetVotesPerDayAsync(int pollId, CancellationToken cancellationToken = default)
    {

        var pollIsExists = await _context.Polls.AnyAsync(x => x.Id == pollId, cancellationToken: cancellationToken);

        if (!pollIsExists)
            return Result.Failure<IEnumerable<VotesPerDayRespone>>(PollErrors.PollNotFound);
        ...
        var votesPerDay = await _context.Votes.Where(x => x.PollId == pollId)
            ...
            .Select(g => new VotesPerDayRespone(
        ...
        return Result.Success<IEnumerable<VotesPerDayRespone>>(votesPerDay);
    }

    public async Task<Result<IEnumerable<VotesPerQuestionResponse>>> GetVotesQuestionAsync(int pollId, CancellationToken cancellationToken = default)
    {
